...
class DefaultWordsRepositoryTest {
    ...
    fun `when words are observed_given there are cached words for the widget_words stored locally`() = runTest(UnconfinedTestDispatcher()) {
        ...
        val wordPair = getRandomWordPair()
    ...
    private fun TestScope.collectWords(widgetId: Widget.WidgetId): List<List<WordPair>?> {
        val words = mutableListOf<List<WordPair>?>()
        backgroundScope.launch {
            repository.observeWords(widgetId).toList(words)
        }
        return words
    }

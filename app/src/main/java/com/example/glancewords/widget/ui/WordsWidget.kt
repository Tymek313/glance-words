...
fun WordsWidgetContent() {
    ...
    Column(GlanceModifier.fillMaxSize().background(GlanceTheme.colors.background).padding(8.dp)) {
        Box(contentAlignment = Alignment.Center, modifier = GlanceModifier.defaultWeight()) {
...
private fun WordList(words: List<Pair<String, String>>, modifier: GlanceModifier = GlanceModifier) {
    LazyColumn(modifier) {
        items(words) { (englishWord, polishWord) ->
            Row(GlanceModifier.fillMaxWidth().background(ImageProvider(R.drawable.rounded_background))) {
                ...
                WordsText(text = englishWord, rowModifier)
                WordsText(text = polishWord, rowModifier)
            }
        }
    }
}
...
private sealed interface WidgetState {
    object InProgress : WidgetState
    object Failure : WidgetState
    class Success(val words: List<Pair<String, String>>) : WidgetState
}

@Composable
fun WordsText(text: String, modifier: GlanceModifier = GlanceModifier, maxLines: Int = Int.MAX_VALUE) {
    Text(text, modifier, style = TextDefaults.defaultTextStyle.copy(color = GlanceTheme.colors.onBackground), maxLines)
}
